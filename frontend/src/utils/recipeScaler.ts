/**
 * Scales ingredient quantity based on target servings vs baseline servings (default 2 or 4).
 */

export function scaleIngredientAmount(amountStr: string, targetServings: number, baselineServings = 4): string {
  ...
  // Match pattern: "500g", "500 g", "1.5 kg", "2 thìa canh", "3 quả"
  ...
  // Fraction format like "1/2 bát" or "1/4 quả"
...
export function scaleCalories(calories: number, targetServings: number, baselineServings = 4): number {
  if (!calories || typeof calories !== 'number') return calories;
  return Math.round((calories / baselineServings) * targetServings);
}

export function scaleMacro(val: number, targetServings: number, baselineServings = 4): number {
